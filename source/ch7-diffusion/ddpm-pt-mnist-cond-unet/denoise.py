...
if __name__ == '__main__':
    ...
    if USE_LORA:
        ...
        for name, layer in model.named_modules():
            ...
            if isinstance(layer, LoraLayer):
                ...
                layer.raw_linear.weight = nn.Parameter(layer.raw_linear.weight.add(lora_weight.T)).to(
                    DEVICE)  # 把Lora参数加到base model的linear weight上
    ...
    batch_cls = torch.arange(start=0, end=10, dtype=torch.long)  # 引导词prompt
    # 逐步去噪得到原图
