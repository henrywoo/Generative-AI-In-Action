...
from cv_torch import get_cv_dataset, DS_PATH_MNIST
import wandb
...
set_seed(1979, has_tf=False, has_torch=True)
...
class SVAE_CNN(nn.Module):
    def __init__(self, embedding_dim=84):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(
                embedding_dim, 128 * 7 * 7
            ),  # Map to 128 channels with 7x7 feature maps
            nn.ReLU(),
            nn.Unflatten(
                1, (128, 7, 7)
            ),  # Unflatten to match ConvTranspose2d input shape
...
class SVAE(SVAE_CNN):
    def __init__(self):
        ...
        y = y / y[-1]
        self.W = torch.tensor(
            np.interp(np.random.random(10**6), y, x), dtype=torch.float32
        )
    ...
    def reparameterize(self, mu):
        dims = mu.size(-1)
        idx = torch.randint(
            0, 10**6, (mu.size(0), 1), dtype=torch.long, device=mu.device
        )
    ...
    def generate(self, num_samples, device, noise_scale=0.05, class_means=None):
        if class_means is None:
            ...
            return self.generate_with_guidance(
                num_samples, device, noise_scale, class_means
            )
    ...
    def generate_rand(self, num_samples, device, noise_scale=0.05):
        with torch.no_grad():
            dims = latent_dim
            idx = torch.randint(
                0, 10**6, (num_samples, 1), dtype=torch.long, device=device
            )
    ...
    def generate_with_guidance(self, num_samples, device, noise_scale, class_means):
        with torch.inference_mode():
            dims = latent_dim
            selected_means = class_means[
                torch.randint(0, class_means.shape[0], (num_samples,))
            ]
...
def plot_recon_loss(recon_loss_history, replace_it, version):
    ...
    plt.plot(
        epochs, recon_loss_history, label="Reconstruction Loss", marker="o", alpha=0.5
    )
    plt.xlabel("Epochs")
    plt.ylabel("Loss")
...
def save_recon_loss_history(recon_loss_history, replace_it, version):
    ...
    try:
        ...
        if len(existing_df[f"v{version}"]) < max_length:
            existing_df[f"v{version}"] = existing_df[f"v{version}"].tolist() + [
                float("nan")
...
def contrastive_loss_cosine(z, labels, margin=0.05, multiplier=500.0):
...
def train(
    ...
    contrastive_loss_value, vmfml_loss_value, kld = [
        torch.tensor(0.0, device=device)
    ] * 3
    batch_idx = 1
    small_constant = 1e-8
    with tqdm(
        total=len(train_loader.dataset), desc=f"Train Epoch {epoch}", unit="samples"
    ) as pbar:
        for batch_idx, (images, labels) in enumerate(train_loader):
            ...
            if contrastive:
                ...
                contrastive_loss_value = contrastive_loss_cosine(
                    z, labels.to(device), multiplier=CONTR_MUL
                )
            if use_vmfml:
                z = quantized.view(quantized.size(0), -1)
                vmfml_loss_value = vmfml_loss_v2(
                    z, labels.to(device), kappa=kappa, num_classes=num_classes
                )
    ...
    print(
        f"====> Epoch: {epoch} Average train loss: {avg_train_loss:.4f},"
    ...
    wandb.log(
        {
            "t/avg_train_loss": avg_train_loss,
    ...
    wandb.log(
        {
            "Reconstructions": [
                wandb.Image(
                    grid, caption="Top: Original images, Bottom: Reconstructed images"
                )
            ]
        }
    )
...
def visualize_latent_space(
    model, test_loader, device, num_classes=NUM_CLASSES, version=0
):
    if latent_dim != 3:
        return
    model.eval()
...
def visualize_reconstructed_digits(
    model, device, latent_dim, version=0, class_means=None
):
    import random
        ...
        for i in range(n):
            for j in range(n):
                ...
                if class_means is None:
                    ...
                    z_sample = (
                        torch.randn(1, latent_dim, device=device) / TEMPERATURE
                        + class_means[random.randint(0, 9)]
                    )
...
def visualize_generated_images(
    model, num_samples, device, noise_scale=0.1, class_means=None, version=0
):
    model.eval()
    with torch.no_grad():
...
def main(args):
    ...
    if os.path.exists(checkpoint_path):
        start_epoch, train_loss_history, val_loss_history = load_checkpoint(
    ...
    visualize_reconstructed_digits(
        model, device, latent_dim, class_means=class_means, version=0
    )
