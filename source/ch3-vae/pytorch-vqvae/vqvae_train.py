...
def train(data_loader, model, optimizer, args, writer):
    for images, _ in data_loader: # for images, _ in tqdm(data_loader, desc="Training"):
...
def validate(data_loader, model, args, writer):
    with torch.no_grad():
...
def main(args):
    ...
    for epoch in tqdm(range(args.num_epochs)):
        ...
        loss, _ = validate(valid_loader, model, args, writer)
        reconstruction = generate_samples(fixed_images, model, args)
        grid = make_grid(reconstruction.cpu(), nrow=8, range=(-1, 1), normalize=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data-folder', type=str, default='data', help='name of the data folder')
    ...
    parser.add_argument('--batch-size', type=int, default=512, help='batch size (default: 128)')
    ...
    parser.add_argument('--num-workers', type=int, default=min(16, mp.cpu_count() - 1), help='number of workers for trajectories sampling (default: {0})'.format(mp.cpu_count() - 1))
