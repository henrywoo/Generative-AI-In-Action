...
model = TiTok(image_size=image_size, patch_size=patch_size, num_classes=1000, dim=latent_dim, depth=6, heads=16, mlp_dim=2048).to(device)
optimizer = optim.Adam(model.parameters(), lr=learning_rate)
criterion = nn.MSELoss()


# Assuming you have a function get_proxy_codes() that returns the proxy codes
def warmup_training(model, dataloader, criterion, optimizer, num_epochs):
    model.train()
    for epoch in range(num_epochs):
        for images, _ in tqdm(dataloader):
            ...
            proxy_codes = get_proxy_codes(images)
            loss = criterion(outputs, proxy_codes)
...
warmup_training(model, train_loader, criterion, optimizer, num_epochs_warmup)
