...
class PositionalEncoding1D(nn.Module):
    ...
    def forward(self, x):
        # x should have the shape (batch_size, seq_len, dim)
        assert x.shape[1:] == self.pos_embed.shape[1:], \
            f"Input tensor shape {x.shape} does not match positional encoding shape {self.pos_embed.shape}"
...
# tokenizer
class TiTok(Module):
    def __init__(self, *, dim=1024, image_size=256, patch_size=32, channels=3, K=32, heads=None, mlp_dim=None,
                 B=None, depth=6, codebook_size=1024, codebook=None, enc_kwargs: dict = dict(),
        ...
        self.positional_encoding = PositionalEncoding1D(dim, num_tokens, cls_token=False)
    ...
    def decode(self, latents):
        ...
        if 0:
            ...
            tokens, mask_packed_shape = pack([latents, mask_tokens], 'b * d')
            tokens = self.decoder(tokens)
            _, tokens = unpack(tokens, mask_packed_shape, 'b * d')
