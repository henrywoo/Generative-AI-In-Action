...
def get_model_and_optimizer(args, ddp, ddp_local_rank=None):
...
def train_one_epoch(model, optimizer, args, scheduler=None, ddp=False):
...
def main():
    ...
    if args.decay_lr:
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.lr_decay_iters, eta_min=args.min_lr)
    else:
        scheduler = None

    # Training loop
    try:
        train_one_epoch(model, optimizer, args, scheduler, ddp)
