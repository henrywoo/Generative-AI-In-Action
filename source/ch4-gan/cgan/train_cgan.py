...
def train(generator, discriminator, data_loader, g_optimizer, d_optimizer, criterion, epochs, z_size, class_num, device,
          output_dir, checkpoint_dir, resume):
    start_epoch = 0
    if resume:
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for images, labels in data_loader:
            ...
            generator.train()
            d_loss = discriminator_train_step(len(real_images), discriminator, generator, d_optimizer, criterion,
                                              real_images, labels, z_size, class_num, device)
            g_loss = generator_train_step(len(real_images), discriminator, generator, g_optimizer, criterion, z_size,
                                          class_num, device)
...
def main():
    ...
    train(generator, discriminator, train_loader, g_optimizer, d_optimizer, criterion, args.epochs, args.z_size,
          class_num, device, args.output_dir, args.checkpoint_dir, args.resume)
