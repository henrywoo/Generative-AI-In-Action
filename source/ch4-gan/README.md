...
In GAN training, it generally doesn't matter significantly whether you train the generator (G) or the discriminator (D) first in each iteration. However, a common practice is:

1. **Train the Discriminator (D)**: Update D with real and generated samples to improve its ability to distinguish between real and fake data.
2. **Train the Generator (G)**: Update G based on the feedback from D, improving its ability to produce realistic samples.

### Typical Steps:
- **Step 1**: Train D on a batch of real data and a batch of fake data generated by G.
- **Step 2**: Train G to maximize the error of D on fake data.

### Reasons:
- Training D first helps ensure it can correctly identify real from fake, providing meaningful gradients for G.
- Training G first could lead to unstable gradients if D is not well-trained initially.

While the order can vary, the sequence above is widely used and recommended for stable GAN training.
...
|     Method      | Token Type | #Tokens |     Train Data     | Codebook Size | rFID | PSNR  | Codebook Utilization |
|:---------------:|:----:|:-----:|:------------------:|:-------------:|:----:|:----:|:--------------------:|
