...
parser.add_argument('--checkpoint-path', type=str, default="./checkpoints/vqgan_best.pt",
...
pt = torch.load(os.path.join("checkpoints", "transformer_best.pt"))
transformer.load_state_dict(pt['model_state_dict'])
...
for i in tqdm(range(n)):
    ...
    f = os.path.join("results", "generation", f"{i}.jpg")
    ensure_folder(f)
    vutils.save_image(sampled_imgs, f, nrow=4)
