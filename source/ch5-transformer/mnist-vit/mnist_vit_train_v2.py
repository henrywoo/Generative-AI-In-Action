...
def main(args):
    ...
    val_loader = DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False, num_workers=4,
                            persistent_workers=True)
    model = ViT(classify_features=10).to(DEVICE)
    ...
    training_losses, validation_losses = train_and_validate_model(model, ema_model, optimizer, scheduler, alpha,
                                                                  train_loader, val_loader, DEVICE, args, best_val_loss)
...
def train_and_validate_model(model, ema_model, optimizer, scheduler, alpha, train_loader, val_loader, device, args,
                             best_val_loss):
    training_losses = []
    validation_losses = []
